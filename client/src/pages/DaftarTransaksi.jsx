    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="deskripsi">Keterangan</label>
          ...
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="jumlah">Jumlah (Nominal)</label>
          ...
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="tanggal">Tanggal</label>
          ...
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="kategori_id">Kategori</label>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
        ...
        <div className="flex flex-wrap gap-2">
           ...
           <select className="border p-2 rounded-md" value={filterCategory} onChange={(e) => { setFilterCategory(e.target.value); setCurrentPage(1); }} >
             ...
             {categories.length > 0 ? ( categories.map((c) => (<option key={c.id} value={c.id}>{c.nama_kategori}</option>)) ) : (<option disabled>Memuat...</option>)}
           </select>
      ...
      {!loading && !error && (
        ...
        <div className="p-5 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => changePage(-1)} disabled={currentPage === 1 || isEmpty} className="border px-4 py-2 rounded-md disabled:opacity-50">Prev</button>
            <span>Halaman {currentPage} dari {totalPages}</span>
            <button onClick={() => changePage(1)} disabled={currentPage === totalPages || isEmpty} className="border px-4 py-2 rounded-md disabled:opacity-50">Next</button>
          </div>
        </div>
        </>
      )}
