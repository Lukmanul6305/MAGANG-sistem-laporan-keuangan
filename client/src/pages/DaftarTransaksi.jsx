    ...
    <div className={`flex flex-col p-5 ${isOpen ? "lg:ml-70" : "lg:ml-20"} transition-all max-w-full`}>
            ...
            {!isEmpty ? (
              transactions.map((item) => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className={`py-2 px-4 text-sm font-medium ${item.tipe === "Pemasukan" ? "text-green-600" : "text-red-600"}`}>Rp {item.jumlah.toLocaleString("id-ID")}</td>
      ...
      <div className="p-5 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => changePage(-1)} disabled={currentPage === 1} className="border px-4 py-2 rounded-md disabled:opacity-50">
            Prev
          </button>
          <span>
            Halaman {currentPage} dari {totalPages}
          </span>
          <button onClick={() => changePage(1)} disabled={currentPage === totalPages} className="border px-4 py-2 rounded-md disabled:opacity-50">
            Next
          </button>
        </div>
      </div>
