...
const Dashboard = ({ isOpen }) => {
 ...
 const lineChartData = monthlyData.map((item) => {
  ...
  return (
    <div className={`flex flex-col p-5 md:p-8 transition-all duration-300 ease-in-out ${isOpen ? "lg:ml-64" : "lg:ml-20"}`}>
        {/* Header */}
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
            <div>
                <h1 className="text-4xl font-bold text-red-800">Dashboard</h1>
                <p className="text-xs text-gray-500">{dateTime.toLocaleDateString("id-ID", { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} | {dateTime.toLocaleTimeString("id-ID")}</p>
            </div>
